 ...
 * All rights reserved. 
 *
 * $id: RequestContext.java 9552 2014-3-12 下午01:28:44 WangLijun$
 */
...
public class RequestContext {
	...
	/**
	 * Default theme name used if the RequestContext cannot find a
	 * ThemeResolver. Only applies to non-DispatcherServlet requests.
	 * <p>
	 * Same as AbstractThemeResolver's default, but not linked in here to avoid
	...
	/**
	 * Request attribute to hold the current web application context for
	 * RequestContext usage. By default, the DispatcherServlet's context (or the
	 * root context as fallback) is exposed.
	 */
	public static final String WEB_APPLICATION_CONTEXT_ATTRIBUTE = RequestContext.class
			.getName() + ".CONTEXT";
	 ...
	 * <p>
	 * This only works with InternalResourceViews, as Errors instances are part
	 * of the model and not normally exposed as request attributes. It will
	 * typically be used within JSPs or custom tags.
	 * <p>
	 * <b>Will only work within a DispatcherServlet request.</b> Pass in a
	 ...
	 * typically be used within JSPs or custom tags.
	 * <p>
	 * If a ServletContext is specified, the RequestContext will also work with
	 * the root WebApplicationContext (outside a DispatcherServlet).
	 * 
	 * @param request
	 *            current HTTP request
	 * @param servletContext
	 *            the servlet context of the web application (can be
	 *            <code>null</code>; necessary for fallback to root
	...
	/**
	 * Create a new RequestContext for the given request, using the given model
	 * attributes for Errors retrieval.
	 * <p>
	 * This works with all View implementations. It will typically be used by
	 ...
	 *            current HTTP request
	 * @param model
	 *            the model attributes for the current view (can be
	 *            <code>null</code>, using the request attributes for Errors
	 *            retrieval)
	 * @see org.springframework.web.servlet.DispatcherServlet
	 * @see #RequestContext(javax.servlet.http.HttpServletRequest,
	 *      javax.servlet.ServletContext, Map)
	 */
	 ...
	 * <p>
	 * If a ServletContext is specified, the RequestContext will also work with
	 * a root WebApplicationContext (outside a DispatcherServlet).
	 * 
	 * @param request
	 *            current HTTP request
	 * @param response
	...
	/**
	 * Initialize this context with the given request, using the given model
	 * attributes for Errors retrieval.
	 * <p>
	 * Delegates to <code>getFallbackLocale</code> and
	 * <code>getFallbackTheme</code> for determining the fallback locale and
	 * theme, respectively, if no LocaleResolver and/or ThemeResolver can be
	...
	protected void initContext(HttpServletRequest request,
		...
		// Fetch WebApplicationContext, either from DispatcherServlet or the
		// root context.
		// ServletContext needs to be specified to be able to fall back to the
		// root context!
		this.webApplicationContext = (WebApplicationContext) request
				.getAttribute(WEB_APPLICATION_CONTEXT_ATTRIBUTE);
		if (this.webApplicationContext == null) {
			this.webApplicationContext = RequestContextUtils
		...
		this.defaultHtmlEscape = WebUtils
				.getDefaultHtmlEscape(this.webApplicationContext
						.getServletContext());
		...
		try {
			this.requestDataValueProcessor = this.webApplicationContext
					.getBean(REQUEST_DATA_VALUE_PROCESSOR_BEAN_NAME,
	 ...
	 * <code>HttpServletRequest.getLocale()</code>.
	 * 
	...
	protected Locale getFallbackLocale() {
		if (jstlPresent) {
			Locale locale = JstlLocaleResolver.getJstlLocale(getRequest(),
					getServletContext());
			if (locale != null) {
				return locale;
			}
	...
	/**
	 * Inner class that isolates the JSTL dependency. Just called to resolve the
	 * fallback locale if the JSTL API is present.
	 */
	private static class JstlLocaleResolver {

		public static Locale getJstlLocale(HttpServletRequest request,
				ServletContext servletContext) {
			...
			if (localeObject == null) {
				...
				if (localeObject == null && servletContext != null) {
					localeObject = Config
							.get(servletContext, Config.FMT_LOCALE);
				}
			}
			return (localeObject instanceof Locale ? (Locale) localeObject
					: null);
		}
	}
	...
	protected Theme getFallbackTheme() {
		ThemeSource themeSource = RequestContextUtils
				.getThemeSource(getRequest());
		if (themeSource == null) {
		...
		if (theme == null) {
			throw new IllegalStateException(
					"No theme defined and no fallback theme found");
		}
		return theme;
	}
	 ...
	 * (named keys with braces <code>{}</code>). For example, send in a relative
	 * URL <code>foo/{bar}?spam={spam}</code> and a parameter map
	 * <code>{bar=baz,spam=nuts}</code> and the result will be
	 * <code>[contextpath]/foo/baz?spam=nuts</code>.
	 * 
	 * @param relativeUrl
	 *            the relative URL part
	 * @param a
	 *            map of parameters to insert as placeholders in the url
	 * @return a URL that points back to the server with an absolute path (also
	 *         URL-encoded accordingly)
	 */
	 ...
	 * <p>
	 * Delegates to the UrlPathHelper for decoding the context path.
	 * 
	 ...
	 * without parameters. This is particularly useful as HTML form action
	 * target, possibly in combination with the original query string.
	 * <p>
	 * <b>Note this implementation will correctly resolve to the URI of any
	 * originating root request in the presence of a forwarded request. However,
	 * this can only work when the Servlet 2.4 'forward' request attributes are
	 * present.
	 * <p>
	 ...
	 * <p>
	 * <b>Note this implementation will correctly resolve to the query string of
	 * any originating root request in the presence of a forwarded request.
	 * However, this can only work when the Servlet 2.4 'forward' request
	 ...
	 *            code of the message
	 * @param args
	 *            arguments for the message as a List, or <code>null</code> if
	 *            none
	 * @param defaultMessage
	 *            String to return if the lookup fails
	 * @return the message
	...
	public String getMessage(String code, List<?> args, String defaultMessage) {
		return getMessage(code, (args != null ? args.toArray() : null),
				defaultMessage, isDefaultHtmlEscape());
	}
	...
	public String getMessage(String code, Object[] args, String defaultMessage,
			boolean htmlEscape) {
		String msg = this.webApplicationContext.getMessage(code, args,
	...
	/**
	 * Retrieve the given MessageSourceResolvable (e.g. an ObjectError
	 * instance), using the "defaultHtmlEscape" setting.
	 * 
	 * @param resolvable
	...
	/**
	 * Is default HTML escaping active? Falls back to <code>false</code> in case
	 * of no explicit default given.
	 */
	 ...
	 */
	public String getMessage(MessageSourceResolvable resolvable,
			boolean htmlEscape) throws NoSuchMessageException {
		String msg = this.webApplicationContext.getMessage(resolvable,
				this.locale);
		return (htmlEscape ? HtmlUtils.htmlEscape(msg) : msg);
	}
	...
	public String getThemeMessage(String code, String defaultMessage) {
		return getTheme().getMessageSource().getMessage(code, null,
				defaultMessage, this.locale);
	}
	...
	public String getThemeMessage(String code, Object[] args,
		...
		return getTheme().getMessageSource().getMessage(code, args,
				defaultMessage, this.locale);
	}
	...
	public String getThemeMessage(String code, List<?> args,
		...
		return getTheme().getMessageSource().getMessage(code,
				(args != null ? args.toArray() : null), defaultMessage,
				this.locale);
	}
	...
	public String getThemeMessage(String code) throws NoSuchMessageException {
		return getTheme().getMessageSource()
				.getMessage(code, null, this.locale);
	}
	...
	public String getThemeMessage(String code, Object[] args)
			throws NoSuchMessageException {
		return getTheme().getMessageSource()
				.getMessage(code, args, this.locale);
	}
	...
	public String getThemeMessage(String code, List<?> args)
			throws NoSuchMessageException {
		return getTheme().getMessageSource().getMessage(code,
				(args != null ? args.toArray() : null), this.locale);
	}
	...
	public Errors getErrors(String name, boolean htmlEscape) {
		...
		boolean put = false;
		if (errors == null) {
			errors = (Errors) getModelObject(BindingResult.MODEL_KEY_PREFIX
					+ name);
	...
	/**
	 * Return the underlying ServletContext. Only intended for cooperating
	...
	public void setRequestDataValueProcessor(
	...
	public void setErrorsMap(Map<String, Errors> errorsMap) {
